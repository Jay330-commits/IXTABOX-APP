...
/** Vague locations: large radius covering full residential areas (meters) */
const MOCK_VAGUE_LOCATIONS: VagueLocation[] = [
  { id: "v1", lat: 59.3293, lng: 18.0686, name: "Stockholm area", coverageRadiusM: 35000, type: "vague" },
  { id: "v2", lat: 59.8586, lng: 17.6389, name: "Uppsala area", coverageRadiusM: 30000, type: "vague" },
  { id: "v3", lat: 57.7089, lng: 11.9746, name: "Gothenburg area", coverageRadiusM: 40000, type: "vague" },
  { id: "v4", lat: 55.6050, lng: 13.0038, name: "Malmö area", coverageRadiusM: 35000, type: "vague" },
  { id: "v5", lat: 59.6110, lng: 16.5446, name: "Västerås area", coverageRadiusM: 32000, type: "vague" },
];
...
function IxtaownerMapContent({
  ...
  const computedBounds = useMemo(() => {
    ...
    // ~111 km per degree lat; extend bounds to include full circle area
    MOCK_VAGUE_LOCATIONS.forEach((loc) => {
      const dLat = loc.coverageRadiusM / 111000;
      const dLng = loc.coverageRadiusM / (111000 * Math.cos((loc.lat * Math.PI) / 180));
  ...
  const mapContent = (
    ...
    >
        ...
        <div className="fixed right-4 z-[10000] flex flex-col gap-2" style={{ top: "100px", bottom: "20px" }}>
          {onLocationChosen && (
            <button
              ...
              className={`rounded-lg px-4 py-2 text-sm font-medium shadow-lg backdrop-blur-sm whitespace-nowrap ${
                pickMode ? "bg-amber-500/90 text-white border border-amber-400" : "bg-black/80 text-white border border-white/20 hover:bg-black/90"
              }`}
            >
      ...
      {selectedLocation && (
        <div className="absolute bottom-4 left-4 right-4 md:left-1/2 md:right-auto md:-translate-x-1/2 md:w-[420px] z-10 rounded-xl border border-white/10 bg-slate-800/95 p-4 shadow-xl backdrop-blur-sm">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <h4 className="font-semibold text-white">{selectedLocation.name}</h4>
              <p className="mt-0.5 text-xs text-gray-400">Address after payment</p>
            </div>
            <button
              type="button"
          ...
          <div className="mt-4">
            {showPaymentStep ? (
              ...
              <div className="space-y-3 rounded-lg bg-slate-800/85 p-3">
                ...
                <div className="rounded-lg border border-white/10 bg-slate-900/60 p-3">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-400">{selectedLocation?.name}</span>
                    <span className="font-semibold text-white">{totalPrice} SEK</span>
                  </div>
                  <p className="mt-1 text-xs text-gray-500">
                    {days} {days === 1 ? "day" : "days"} × {MOCK_PRICE_PER_DAY} SEK
                  </p>
                </div>
              ...
              <>
            {/* Box details – location & availability times (owner sets) */}
            <div className="mb-3 space-y-2 rounded-lg border border-white/10 bg-slate-800/60 px-3 py-2">
              <div>
                <p className="text-xs font-medium text-cyan-200">Location</p>
                <p className="text-xs text-gray-300">{selectedLocation.name} · ~{(selectedLocation.coverageRadiusM / 1000).toFixed(1)} km area</p>
              </div>
              {(() => {
              ...
              <div>
                <p className="text-xs font-medium text-cyan-200">Price</p>
                <p className="text-xs text-gray-300">{MOCK_PRICE_PER_DAY} SEK / day</p>
              </div>
            </div>
            ...
            {startDate && endDate && (
              <div className="mt-4 rounded-lg bg-slate-800/85 p-3">
                ...
                <p className="mt-2 text-xs text-gray-500">
                  Complete payment to get owner contact and pickup address.
                </p>
                <button
                  type="button"
                  onClick={handlePay}
